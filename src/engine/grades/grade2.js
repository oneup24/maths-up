...
'2M1':[
  ...
  ()=>{var n1=nm(),n2=nm();while(n1===n2)n2=nm();var m1=ri(50,200),cm1=ri(10,90);var m2=ri(50,200),cm2=ri(10,90);var d1=m1*100+cm1,d2=m2*100+cm2;if(d1===d2){m2+=1;d2=m2*100+cm2;}var who=d1>d2?n1:n2;var diff=Math.abs(d1-d2);var dSign=ri(0,1)===0?-1:1;var dDelta=ri(1,9)*10;var wrongDiff=dSign>0?diff+dDelta:diff-dDelta;if(wrongDiff<=0||wrongDiff===diff){wrongDiff=diff+dDelta;}var dAge=ri(7,9);return{d:3,tp:'mc',q:n1+' 今年 '+dAge+' 歲，跑了 '+m1+' 米 '+cm1+' 厘米。'+n2+' 跑了 '+m2+' 米 '+cm2+' 厘米。誰跑得較遠？遠多少厘米？',isMC:true,opts:[{l:'A',v:who===n1?(n1+','+diff):(n1+','+wrongDiff),c:who===n1},{l:'B',v:who===n2?(n2+','+diff):(n2+','+wrongDiff),c:who===n2},{l:'C',v:'一樣遠',c:false}],a:who===n1?'A':'B',trap:'年齡（'+dAge+'歲）',s:['🔍 年齡與距離無關。',n1+'：'+m1+'米'+cm1+'厘米 = '+(m1*100+cm1)+' 厘米',n2+'：'+m2+'米'+cm2+'厘米 = '+(m2*100+cm2)+' 厘米','相減：|'+d1+' − '+d2+'| = '+diff+' 厘米'],sc:3}},
