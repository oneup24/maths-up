    ...
    <AnimatePresence>{isMarked&&(
      <motion.div initial={{opacity:0,y:-20}} animate={{opacity:1,y:0}} className="bg-white/90 backdrop-blur-sm rounded-2xl p-4 mb-3 border-2 border-[#e3e8ee]" style={{boxShadow:'rgba(0,55,112,0.08) 0 8px 24px, rgba(0,55,112,0.04) 0 2px 6px'}} aria-live="polite">
        ...
        <div className="text-center my-3">
          ...
          <span className="text-sm font-light tabular-nums text-[#64748d] mt-1 inline-block">{animPct}%</span>
          <div className="mt-2">
            ...
            <p className={"text-sm font-light mt-1 "+fb.c}>{fb.m}</p>
        ...
        <div className="border-t pt-2 space-y-1">
          {sections.map((sec,si)=>{var ss=secScores(si);var sp=sec.total>0?Math.round(ss/sec.total*100):0;return(
            <div key={si} className="flex items-center gap-2 text-sm">
              <span className="font-light text-[#64748d] w-6">{sec.label}.</span>
              ...
              <span className="text-xs font-light tabular-nums w-12 text-right">{ss}/{sec.total}</span>
            </div>
          )})}
        </div>
        {topicSummary.length>0&&(
          <div className="mt-3 rounded-xl p-3 bg-[#f6f9fc]">
            <h4 className="font-light text-sm text-[#0d253d] mb-2">📊 {lang==='zh'?'各單元表現':'Topic Performance'}</h4>
              ...
              {topicSummary.map(t=>{var tcls=t.pct>=80?'border-l-emerald-500':t.pct>=50?'border-l-amber-500':'border-l-red-500';var txtcls=t.pct>=80?'text-emerald-600':t.pct>=50?'text-amber-600':'text-red-500';var barcls=t.pct>=80?'bg-emerald-500':t.pct>=50?'bg-amber-500':'bg-red-400';var emoji=t.pct>=80?'✅':t.pct>=50?'⚠️':'❌';return(
                <div key={t.id} className={"bg-white rounded-lg px-3 py-2 border-l-4 "+tcls}>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-light text-[#0d253d] flex-1 truncate">{t.name}</span>
                    <span className={"text-xs font-light tabular-nums shrink-0 "+txtcls}>{t.correct}/{t.total} ({t.pct}%)</span>
                  </div>
